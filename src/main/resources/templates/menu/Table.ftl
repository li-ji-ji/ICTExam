...
layui.use(['table','laypage','layer'], function(){
  ...
  var tableObj = table.render({
	    elem: '#menuTable'
	    ,height: 'full-80'
	    ,url: '${base}/menuAPI/getMenuAll' //数据接口
	    ,page: true //开启分页
	    ,toolbar: '#toolbarDemo' //开启头部工具栏，并为其绑定左侧模板
	    ,cols: [[ //表头
	    	{type:'checkbox', fixed: 'left'}
	        ,{field: 'id', title: '编号',align:'center', width:80, sort: true, fixed: 'left'}
	        ,{field: 'menuName', title: '菜单名称',edit: 'text', align:'center',width:120}
	        ,{field: 'parentName', title: '父级菜单名称',align:'center', width:120, sort: true}
	        ,{field: 'parentId', title: '父级菜单编号',align:'center', width:160} 
	        ,{field: 'status', title: '菜单状态',align:'center', width:120,templet: '#statusCheck', unresize: true}
	        ,{field: 'url', title: '跳转链接',edit: 'text',align:'center', width: 180, sort: true}
	    ...
	    table.on('tool(menuTable)', function(obj){
	      ...
	      if(obj.event === 'detail'){
	        ...
	        layer.confirm('真的删除行么', function(index){
	        	console.log(obj)
	  	    $.ajax({
		    	"url" : "${base}/menuAPI/delMenuOneById",
		    	"data":{
		    		id : obj.data.id
		    		},
		    		"type" : "get",
